...
The **Translation Agent** view under **Translation → Flowcourier** can also translate **dictionary items** — free in every edition. Use *Translate dictionary items…*, pick the items from the dictionary tree, pick the target language, and the default-language values are translated and saved. Existing target values for the picked items are overwritten; an item whose translation fails is left untouched (dictionary lookups fall back to the default language at runtime anyway). On licensed installs [termbase](/docs/ai/guides/translation-pro/) terms are enforced, same as for content.
