...
- **Structured data (JSON-LD)** injected into every rendered page's `<head>`.
- **AI crawler analytics** in the backoffice — with built-in **verification** that blocks vulnerability scanners spoofing AI user agents.
...
5. Watch the AI crawlers arrive
    ...
    Open **Analytics → AI Crawlers** — or **Settings → Flowcourier → AEO Dashboard** on a site without [Flowcourier Analytics](/docs/analytics/start-here/get-started/). Visits from GPTBot, ClaudeBot, PerplexityBot and 150+ other AI crawlers are recognised and counted — which bot, which pages, and whether they fetched HTML or your Markdown surface. Each visit is also **verified against the vendor's official mechanism** where one exists (published IP ranges or reverse DNS), and scanners spoofing an AI user agent are blocked automatically — manage this on the Blocking page under **Settings → Flowcourier**. Privacy-safe by default: the analytics store no IP addresses unless you opt in, and raw user agents are kept only as forensics on spoofed requests — see [Privacy (GDPR)](/docs/aeo/guides/crawler-verification-and-blocking/#privacy-gdpr) for details. See [AI crawler analytics](/docs/aeo/guides/ai-crawler-analytics/) and [Crawler verification & blocking](/docs/aeo/guides/crawler-verification-and-blocking/).

    ![AI crawler hits per day, traffic share per operator, hits per crawler and top pages](../../../../assets/aeo/AEO-Dashboard.png)

</Steps>

:::caution[Behind Cloudflare or another CDN?]
Two extra minutes of setup: newer Cloudflare zones **block AI crawlers by
default**, and behind any CDN the app sees the edge's address instead of the
crawler's, so verification can't run. Allow the crawlers at the edge and set
`ClientIpHeader` (Cloudflare: `CF-Connecting-IP`) — the full checklist is in
[Cloudflare & CDNs](/docs/aeo/guides/cloudflare-and-cdns/).
:::
...
- Track [AI referrals & questions answered](/docs/aeo/guides/ai-referrals/) — which pages AI answers cite and convert into real visits.
- Running behind a CDN? [Cloudflare & CDNs](/docs/aeo/guides/cloudflare-and-cdns/) covers edge bot settings, real client addresses and origin lockdown.
...
- Something unexpected? The [FAQ](/docs/aeo/guides/faq/) answers the questions we hear most.

## Measure before & after

<LinkCard
  title="Free website analyzer"
  description="Paste a URL and get technical, SEO, content and AI-readiness scores with prioritised fixes — in about a minute. Run it before installing AEO for a baseline, then again after to see the difference."
  href="https://ai-readiness.flowcourier.com/analyzer"
/>
