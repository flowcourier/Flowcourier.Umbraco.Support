...
Open **Analytics → AI Traffic** in the backoffice. With [Flowcourier Analytics](/docs/analytics/start-here/get-started/) installed this view is a tab in the **Analytics** section; on a site running AEO alone it is under **Settings → Flowcourier → AEO Dashboard**.

It has four tabs — **Overview**, **Top pages**, **Crawler log** and **[Referrals](/docs/aeo/guides/ai-referrals/)** — sharing one date range with every other Flowcourier Analytics view. Crawler fetch → assistant citation → human arrival is one funnel, so it is one view.
...
![AI crawler hits, distinct crawlers, HTML vs Markdown responses, hits per day, traffic share per operator and hits per crawler](../../../../assets/aeo/AEO-Dashboard.png)

[Referrals](/docs/aeo/guides/ai-referrals/) is the fourth tab of this view. The [Blocking](/docs/aeo/guides/crawler-verification-and-blocking/) page, the [Structured Data](/docs/aeo/guides/structured-data/) form and the read-only settings page are always under **Settings → Flowcourier** — they are configuration, not measurement — in a group named **AEO** when Flowcourier Analytics is installed and **AEO Dashboard** when it is not.
...
- **Crawler log** — the raw log: timestamp, crawler, path, verification
...
| `ClientIpHeader` | `null` | Header carrying the real client address when a trusted edge injects one (e.g. Cloudflare's `CF-Connecting-IP`); default uses the connection's remote address. The package never parses `X-Forwarded-For` itself. Behind a CDN this setting is essential — see [Cloudflare & CDNs](/docs/aeo/guides/cloudflare-and-cdns/). |
