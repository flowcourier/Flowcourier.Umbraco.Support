...
Every request whose user agent claims a known AI crawler is **verified against
the vendor's official mechanism**, spoofers are **blocked with `403`** (on by
default), and repeat offenders escalate onto a persistent block list you manage
in the backoffice.
...
Open **Settings → Flowcourier → AEO Dashboard → Blocking** (the group is named **AEO** on a site that also runs Flowcourier Analytics):
...
- **Block list** — the durable record, stored in the database: addresses that
  kept spoofing (hit the strike threshold) or that you blocked manually. They
  stay blocked across restarts until `DurableBlockRetentionDays` after their
  last offense, and only for requests wearing an AI-crawler user agent. Rows
  shown as "x/3 strikes" are offense records still counting toward a block,
  not blocks yet. Each row has a **Remove** button, and you can block any IP
  address manually.
- **Why is an address on the list?** Every strike records its evidence — when
  it happened, which crawler the request claimed to be, and which verification
  mechanism proved the claim false (published IP ranges or reverse DNS). The
  **Why?** button on a block-list row opens that history in plain language,
  e.g. *"claimed GPTBot, but the address is outside the vendor's published IP
  ranges."* The history keeps the last 10 offenses per address; rows recorded
  before AEO 17.5.7 (and manual blocks) have no history.
...
![The Crawlers tab in Block-all mode: a "No AI entry" banner, an "Allow all crawlers" button to switch back, and every crawler showing Blocked with a per-crawler Allow button](../../../../assets/aeo/AI-Crawler-Blocking-All-AI-Crawlers-are-blocked.png)
...
Verification compares the *connection's* address, so behind Cloudflare (or any
CDN) the app sees the edge's IP instead of the crawler's. AEO recognises known
edge networks (Cloudflare's published ranges are built in) and **fails open**:
crawlers read as *unverifiable* and are never blocked or striked — until you
set `CrawlerAnalytics:ClientIpHeader` (Cloudflare: `CF-Connecting-IP`), which
restores full verification. The **"AEO Client IP / Forwarded Headers" health
check** (Settings → Health Checks) tells you which case you're in and the
exact setting to add. The full walkthrough — edge bot settings included — is
in the dedicated [Cloudflare & CDNs](/docs/aeo/guides/cloudflare-and-cdns/)
guide.

Behind a plain reverse proxy or load balancer (nginx, HAProxy, IIS ARR),
configure ASP.NET Core's
[Forwarded Headers middleware](https://learn.microsoft.com/aspnet/core/host-and-deploy/proxy-load-balancer)
in the host site instead — AEO deliberately never parses `X-Forwarded-For`
itself.
...
| `VerifiedBotHeader` | `null` | Header injected by a trusted edge that already verified the crawler — recorded as *verified* (method `header`), IP-list/rDNS checks skipped. Only use a header your edge strips from client requests. |
| `TrustedEdgeCidrs` | Cloudflare's published ranges | CIDRs of known CDN edge networks. Requests from these fail open as *unverifiable* while no `ClientIpHeader` is set (see [Cloudflare & CDNs](/docs/aeo/guides/cloudflare-and-cdns/)). Configured entries are **added** to the defaults. |
| `TrustedEdgeIpListUrls` | `cloudflare.com/ips-v4`, `/ips-v6` | Published edge-range lists refreshed at runtime on top of the static CIDRs. Configured entries are **added** to the defaults. |
