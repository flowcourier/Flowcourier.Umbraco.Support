...
| `LlmsFullMaxSizeKb` | `5120` | Size cap for `/llms-full.txt` in KB (UTF-8 bytes, 5 MB by default). Pages append in stable URL order; a page that would burst the budget is skipped (one huge page can't evict the rest), and a truncated file ends with a footer stating how many pages were omitted. `0` or negative disables the cap. |
...
Under **Settings → Flowcourier → AEO Dashboard → Settings** (the group is named **AEO** on a site that also runs Flowcourier Analytics) you'll find a read-only page showing the effective configuration of the running site.
